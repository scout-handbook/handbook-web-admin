...
function lessonEditMutexExtend(id: string): void
{
	...
	const actionQueue = new ActionQueue([new Action(CONFIG["api-uri"] + "/v0.9/mutex/" + encodeURIComponent(id) , "PUT", undefined, undefined, exceptionHandler)]);
...
function sendBeacon(id: string): void
{
	...
	{
		navigator.sendBeacon(CONFIG["api-uri"] + "/v0.9/mutex-beacon/" + encodeURIComponent(id)); // eslint-disable-line compat/compat
	}
}
...
function renderLessonEditView(id: string, markdown: string, noHistory: boolean): void
{
	...
	const saveActionQueue = new ActionQueue([new Action(CONFIG["api-uri"] + "/v0.9/lesson/" + encodeURIComponent(id) , "PUT", saveLessonPayloadBuilder, [ActionCallback.RemoveBeacon], saveExceptionHandler)]);
	const discardActionQueue = new ActionQueue([new Action(CONFIG["api-uri"] + "/v0.9/mutex/" + encodeURIComponent(id) , "DELETE", undefined, [ActionCallback.RemoveBeacon, ActionCallback.DismissSpinner], discardExceptionHandler)]);
...
{
	request(CONFIG["api-uri"] + "/v0.9/lesson/" + encodeURIComponent(id), "GET", {}, function(response: RequestResponse): void
	{
