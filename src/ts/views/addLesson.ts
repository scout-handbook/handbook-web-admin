...
function showLessonAddView(field?: string): void
{
	...
	const aq = new ActionQueue([new Action(CONFIG["api-uri"] + "/v1.0/lesson", "POST", addLessonPayloadBuilder, [ActionCallback.FillID])])
	if(field)
	{
		aq.actions.push(new Action(CONFIG["api-uri"] + "/v1.0/lesson/{id}/field", "PUT", function(): Payload {return {"field": encodeURIComponent(field)};}))
	}
